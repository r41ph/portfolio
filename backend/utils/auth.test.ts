...
describe("validateJSONToken", () => {
  test("should return true for a valid token", () => {
    ...
    try {
      const result = validateJSONToken(token) as JwtPayload;
  ...
  test("should return false for an invalid token", () => {
    const token =
      "INVALID_zI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VybmFtZSI6InI0MXBoIiwiaWF0IjoxNzAwNTY2MjU3LCJleHAiOjE3MDA1NjYyNzJ9.D4L0ygw8_eq6X6jUh6Yco4rDXxN6iGagFpDsYbR7CeA";

    try {
      validateJSONToken(token) as JwtPayload;
